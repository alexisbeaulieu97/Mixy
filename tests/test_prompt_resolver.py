...
@pytest.mark.parametrize("prompt_value, expected", [
    ('', DEFAULT_VALUE_MULTI),
    ('one,two,three', ['one', 'two', 'three']),
    ('four,five,six', ['four', 'five', 'six']),
    ('one, two, three', ['one', 'two', 'three']),
])
def test_multi(mocker: MockerFixture, resolver: PromptResolver, prompt_value: str, expected: list[Any]):
...
def test_secret(mocker: MockerFixture, resolver: PromptResolver, prompt_value: str, expected: str):
...
def test_choice(mocker: MockerFixture, resolver: PromptResolver, prompt_value: str, expected: Any):
