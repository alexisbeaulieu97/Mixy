...
The VariableResolver SHALL resolve variable values in the following precedence order (lowest to highest): variable defaults from definition, environment variables (`MIXY_VAR_` prefix), vars file values, per-source value overrides, CLI `--var` overrides. Interactive prompting fills in remaining unresolved required variables.
